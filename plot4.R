...
# setting parameters to plot many graphs
par(mfrow=c(2,2))

#plotting Global Active Power
with(hpcFeb,{
  plot(hpcFeb$Time,as.numeric(as.character(hpcFeb$Global_active_power)),type="l",  xlab="",ylab="Global Active Power")  
  plot(hpcFeb$Time,as.numeric(as.character(hpcFeb$Voltage)), type="l",xlab="datetime",ylab="Voltage")
  ...
  plot(hpcFeb$Time,as.numeric(as.character(hpcFeb$Global_reactive_power)),type="l",xlab="datetime",ylab="Global_reactive_power")
})
